...
function province1() {
    var e = document.getElementById("fahuoprovince");
    for (var i = 0; i < provinceName.length; i++)
        e.options.add(new Option(provinceName[i], provinceName[i]));
    $(".queryprice").trigger("click");
}

function province2() {
    var e = document.getElementById("shouhuoprovince");
    for (var i = 0; i < provinceName.length; i++)
        e.options.add(new Option(provinceName[i], provinceName[i]));
    $(".queryprice").trigger("click");
}

function cityName1(n) {
    var e = document.getElementById("fahuocity");
...
function cityName2(n) {
    var e = document.getElementById("shouhuocity");
    for (var i = e.options.length; i > 0; i--) e.remove(i);
    if (n == 0) return;
    var a = eval("city" + n); //得到城市的数组名
    for (var i = 0; i < a.length; i++) e.options.add(new Option(a[i], a[i]));
...
function loadcity() {
    province1(); //初始时给省名下拉菜单赋内容
    province2(); //初始时给省名下拉菜单赋内容
}